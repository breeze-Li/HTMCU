...
/*[待完成]:
【低功耗模式及上电WiFi发送逻辑】[wifi上电发送，发送完断电以控制功耗]
【配网超时及其过程中的指示信息】
*/
#include "BA45F5250.h"
...
volatile uc stop_WiFi = 0;			//停止WiFi上传
...
volatile uc Upload_Bits;		//上传位
struct {
    uc power_off :1;	//断电状态 	
    uc power_on :1;		//上电状态	
    uc send :1;  		//正在发送
    uc accept :1;  		//正在接受
    uc distributing :1;  	//正在配网
	uc connect :1;  		//已连接网络
	uc reply:1;			//回复
}ws={0,0,0,0,0,0,0};//wifi_status
...
void keyDetect(void);
void Upload(void);
long int add(int NumofPara, ...);
void Encode_Bit(uc i);
...
//				sendstring("hello world");
				function();
#ifdef DEBUG
				if(RD_EE(SetupNetStateAdd)  && stop_WiFi > 0){
					stop_WiFi--;
				}else {
					wifi_upload(ch4_value);
				}
...
//	{	temp = va_arg(pointer, long int);
//		sum += temp;	//通过va_arg函数来访问可变参数，返回类型为int 
			...
			if( !FireErr && !FullErr && !BaseErr){
				...
				if(IsFire==0){
					if(FireCount == 0xFF) {
						IsFire=1; 
						//自检时不响，要提示故障
						if(!Key_S.long_flaged) Buzzerflag = 1;
            ...
            if(j==ComRCah[ComRCah[4]+5]){
            	...
            	switch(ComRCah[5]){
					...
					case 0x89:		//读采样
						...
						break;
					case 0x97:
						SendByte(RD_EE(SetupNetStateAdd));
						SendByte( _pa2 );
					...
					case 0x99:
						Upload_Bits = ComRCah[6];
						sendstring("current_char:");
						//DelaymS(50);
						int2str(ComRCah[6]);
						break;
...
void Upload(void){
	...
	static uc step = 0;
	static uc wait = 0;
	static bit Up_flag = 0;
/*
	sendstring("Upload_Bits = ");
	int2str(Upload_Bits);
//	SendByte(0xFA);
//	SendByte(Upload_Bits);
	sendstring("FirstIn = ");
	int2str(FirstIn);
//	SendByte(0xFB);
//	SendByte(FirstIn);
...
*/
	if(Upload_Bits){
		if(step == 0){
			POWER_WIFI = 1;
			if(FirstIn == 0){
				if(RD_EE(SetupNetStateAdd) == 1){	//曾经配网
					FirstIn = 1; wait = 30;
				} else {
					FirstIn = 1; wait = 120;
				}
...
*/
		...
		if(Up_flag){
			if(Upload_Bits | 1){wifi_upload(test); DelaymS(50);}
			if(Upload_Bits | 2){wifi_upload(ch4_state); DelaymS(50);}
			if(Upload_Bits | 4){wifi_upload(ch4_value); DelaymS(50);}
			if(Upload_Bits | 8){wifi_upload(signal); DelaymS(50);}
			if(Upload_Bits | 16){wifi_upload(life); DelaymS(50);}
			if(Upload_Bits | 32){wifi_upload(mute); DelaymS(50);}
			if(Upload_Bits | 64){wifi_upload(fault); DelaymS(50);}
			if(Upload_Bits | 128){wifi_upload(0); DelaymS(50);}
			Upload_Bits=0;
			Up_flag = 0;
...
void Encode_Bit(uc i){
	switch(i){
		...
		case signal:
			Upload_Bits |= 8;break;
		case life:
			Upload_Bits |= 16;break;
		case mute:
			Upload_Bits |= 32;break;
		case fault:
			Upload_Bits |= 64;break;
		default:
			Upload_Bits |= 128;break;
	}		
}
...
int Getch4_value(void){
	...
	return ((Percentage >= 0) && (Percentage <= 1000)) ? Percentage : 1000;
}
...
void sendstring(char *p){
	//发送ASCII
	while(*p != '\0') SendByte(*p++);
}
void int2str(uc p){
	//正型转为字符型发送，调试用
	if(p%100) SendByte( p/100 + 0x30);
	if(p%10) SendByte( (p%100)/10 +0x30);
	SendByte( p%10 +0x30);
	sendstring("  ");
}
