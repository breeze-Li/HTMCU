...
volatile unsigned int op_value;				//当前采样值
volatile unsigned int reference_value;		//电位器ad值
volatile unsigned int initial_value;			//初始值
volatile unsigned int fire_increment;		//火警增量
volatile unsigned int fire_range;			//火警量程
...
void function(void){
    ...
    sum = sum - OpAveVal[0];
    sum = sum + a;
    OpAveVal[0] = (sum >> 2); 	//4平均
    OpAveVal[1] = (sum & 3);  	///这里取最后两位
