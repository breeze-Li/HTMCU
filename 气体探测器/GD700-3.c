...
//2022年1月6日：串口通讯框架
//2022年2月11日：参照原518，读取AD值（使用12位运放）
...
struct S {
    unsigned int current_OPAvalue;		//当前运放值
    unsigned int current_refvalue;		//当前电位器值
    unsigned int Base_value;     // 初始值
    unsigned int Fire_value;     // 火警标定
    unsigned int Full_value;     // 100%LEL
};
struct S Valves={0,0,0,0,0};
volatile unsigned int OpAveVal[2] = {0};//运放平均值
volatile unsigned int RefAveVal[2]= {0};//基准平均值
