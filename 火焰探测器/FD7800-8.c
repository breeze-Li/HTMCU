...
#define 	MIN_CURRENT	0X0A					//电流最小值
#define		MAX_CURRENT 0X16					//电流最大值
...
volatile uc gengral_Array[20] = {0};	//普通数组(记录最后20秒数据，写入EE)
volatile uc Sum_Array[statistics] = {0};		//判据2总数数组
...
//--------------------------------------//
	...
	if(trigger_flag && (!calibration_flag)){	//非标定状态开始触发判据
		...
		//计算总数
		temp = Add_Array(Sum_Array, pulses_count, total_pause_time);
		if(temp >= total_pause) fire_flag1=1;
		//时间窗数据总数
		//进pulses_count，出temp，一进一出。
