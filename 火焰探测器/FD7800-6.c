...
//--------------------------------------//
	...
	if(trigger_flag){				//开始触发判据
		...
		if(++count<15){
			...
			if(total_pause_count>=total_pause) fire_flag=1;
		}else{
			count=0;
			total_pause_count=0;
		}
		...
		if(alarm_period_count>=alarm_period) fire_flag=1;
		if(fire_flag){
			//报警
			OpenLED				//亮灯
			_papu1=1;			//上拉PA1
...
//	SendByte(sensitivity_value);
//	SendByte(sensitivity_value_old);
